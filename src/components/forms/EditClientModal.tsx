...
export const EditClientModal: React.FC<EditClientModalProps> = ({
  ...
  // Charger les plans de paiement et initialiser le formulaire
  useEffect(() => {
    ...
    if (isOpen && client) {
      ...
      setFormData({
        ...
        paymentPlanId: client.paymentPlan?.id || 1,
        distributorId: client.distributor?.id || 1,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const updatedClient = await clientsService.updateClient(client.id, clientData);
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-end space-x-3 pt-4 border-t theme-border-primary theme-transition">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-6 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 theme-transition disabled:opacity-50 flex items-center"
              >
                {loading ? (
                  <>
